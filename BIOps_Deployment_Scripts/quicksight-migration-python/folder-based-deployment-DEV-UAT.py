...
# dashboard ID flag for IAM policy
Dashboard_ID_prefix_flag = False # whether to add 'AAA-' prefix to dashboard ID
Dashboard_ID_prefix = 'AAA-'

# visual embedding specific configuration
visual_embed_prefix = 'visual-embed'
contentID_bucket = 'migration-scripts'
...
def get_target_dashboard_id(source_dashboard_id, source_dashboard_name):
    ...
    if Dashboard_ID_prefix_flag and Dashboard_ID_prefix not in target_dashboard_id:
        target_dashboard_id = Dashboard_ID_prefix + target_dashboard_id
...
def migrate_dataset(source_dataset_ID, source_ses, target_ses):
    logger.info('Migrating dataset: from %s to %s', source_dataset_ID, get_target_id(source_dataset_ID))
    try:
        ...
        # print(Exception)
        faillist.append({"Dataset": source_dataset_ID, "Error": Exception})
        return
    ...
    if 'RowLevelPermissionDataSet' in res['DataSet']:
        ...
        try:
            ...
            faillist.append({"Dataset": source_dataset_ID, "RLS": rls_id, "Error": str(Exception)})
            return
        ...
        RowLevelPermissionDataSet['Arn'] = get_target_id(RowLevelPermissionDataSet['Arn']).replace(sourceaccountid, targetaccountid)
    ...
    if get_target_id(source_dataset_ID) not in already_migrated:
        logger.info('creating new dataset: %s', get_target_id(source_dataset_ID))
        try:
            newdataset = qu.create_dataset(target_ses, get_target_id(source_dataset_ID), get_target_id(name), PT, LT,
                                        res['DataSet']['ImportMode'], target['datasetpermission'], ColumnGroups,
            ...
            newsetslist.append(get_target_id(source_dataset_ID))
        except Exception as e:
            # print('failed: ' + str(e))
            faillist.append({"DataSetId": source_dataset_ID, "Name": name, "Error": str(e)})
            return

    if get_target_id(source_dataset_ID) in already_migrated:
        logger.info('updating already migrated dataset: %s', get_target_id(source_dataset_ID))
        try:
            newdataset = qu.update_dataset(target_ses, get_target_id(source_dataset_ID), get_target_id(name), PT, LT,
    ...
    qu.create_folder_membership(target_ses, target_folder_ID, get_target_id(source_dataset_ID), 'DATASET')
    logger.info('added DATASET {} to target folder'.format(get_target_id(source_dataset_ID)))


for mds in dataset_migrate_list:
    migrate_dataset(mds, sourcesession, targetsession)
    # print(newsetslist)
    # print(mds)
