...
def describe_dashboard_definition(session, dashboard):
    qs_client = session.client('quicksight')
    sts_client = session.client("sts")
    account_id = sts_client.get_caller_identity()["Account"]
    try:
        ...
        return 'Faild to describe dashboard definition: '+str(exc)
    else:
        return response
...
def update_theme(session, theme_id, name, base_theme_id, configuration):
    qs_client = session.client('quicksight')
    sts_client = session.client("sts")
    account_id = sts_client.get_caller_identity()["Account"]
