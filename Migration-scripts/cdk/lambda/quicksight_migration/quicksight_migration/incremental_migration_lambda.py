...
def migrate(
    ...
    newsourceslist = []
    datasource_failed = []
    for datasource in migration_list:
    ...
    for mds in migration_list:
        if mds['DataSetId'] not in already_migrated:
            try:
                ...
                dataset_failed.append({"Dataset": mds, "Error": str(Exception)})
                continue

            name = mds['Name']
            dataset_id = mds['DataSetId']
    ...
    for analysis in source_analyses:
        if analysis['Status'] != 'DELETED':
            sourceanalysis_all.append(analysis)
    ...
    for analysis in sourceanalysis_all:
